...
class CssFileModifier extends CssModifier implements FileModifierInterface
{
    /**
    ...
    public function processFiles(array $files, Template $template): string
    {
        ...
        if ($outputDir === '') {
            throw new \RuntimeException('CssFileModifier: output_dir is not configured.');
        }
        ...
        if (!$this->isFresh($targetFile, array_values($files))) {
            ...
            foreach ($files as $component => $f) {
                ...
                // Strip <style> wrapper tags
                $content = (string) preg_replace('/<\/?style[^>]*>/i', '', $content);
